    ...
    <fieldset>
        ...
        <meta name="csrf-token" content="{{ csrf_token() }}" />
          <div class="form-group">
            ...
            <input type="hidden" id="exam_id" name="exam_id" value="{{$class_id['id']}}{{$data['exam_id']}}">
            <label for="exam-name" class="col-form-label">Name:</label>   
            <input type="text" name="name" id="name" value="{{$data['name']}}" placeholder="{$class_id['name']}" class="code" disabled>
            @error('name')
...
<script>
  ...
  $result = \App\Models\ExamSubject::where([['exam_id','=','1'],['subject_id','=',' 1']])->value('total'); 
  $max = \App\Models\ExamSubject::where([['exam_id','=','1'],['subject_id','=',' 1']])->value('max_marks'); 
  // $result = \DB::table('exam_subjects')->where([['exam_id','=','1'],['subject_id','=',' 1']])->pluck('max_marks','total'); 
  ...
  let marks=Number(document.getElementById('max').value);
  // let marks=Number("{{$data['marks']}}");

  // let marks=$('max').val("{{$data['marks']}}");
  if(marks>=result && marks<=max){
    // $("#pass").val('pass');
    document.getElementById('pass').value='pass';
  }
  else if(marks<result){
    document.getElementById('pass').value='fail';
  }else{
    document.getElementById('pass').value='invalid';
  }
...
function myfunction() {
  // if(id['value']==''){
  // var class_id={{$class_id['class_id']}};
  // }
  // else{
  // var class_id={{$class_id['class_id']}};
  // }
  ...
  $.ajax({
    ...
    type:'get',
    data:{class_id: "{{$class_id['class_id']}}", student_id: "{{$data['id']}}"},
    dataType: 'json',
      ...
      console.log(data)
            ...
            for(var i=0; i<len; i++){
              ...
              if('{{$data['marks']}}'==''){
                ...
                var tr_str = "<tr>"+
                    "<td align='center'><input type='hidden' name='subject_id' id='check' value="+id+" required>"+ name + "</td>" +
